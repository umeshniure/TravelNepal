<%-- 
    Document   : bookcover-insert-form
    Created on : Sep 6, 2022, 2:23:02 PM
    Author     : Umesh
--%>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 p-4 gap-4 text-black dark:text-white">
                    <div class="mt-5 md:col-span-2 md:mt-0">
                        <c:if test='${action == "update"}'>
                            <label class="block text-xl font-medium text-gray-700">Update cover type</label>
                        </c:if>
                        <c:if test='${action == "insert"}'>
                            <label class="block text-xl font-medium text-gray-700">Add new cover type</label>
                        </c:if>
                                    ...
                                    <div class="grid grid-cols-6 gap-6">
                                        <div class="col-span-6 sm:col-span-4">
                                            ...
                                            <c:if test='${action == "update"}'>
                                                ...
                                                <input type="number" name="id" value="${bookCover.id}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm select-none cursor-not-allowed" readonly>
                                                <input type="hidden" name="action" value="updateBookCoverType">
                                            </c:if>
                                            <c:if test='${action == "insert"}'>
                                                <input type="hidden" name="action" value="insertBookCoverType">
                                            </c:if>
                                        </div>
                                        <div class="col-span-6 sm:col-span-4">
                                            <label for="email-address" class="block text-sm font-medium text-gray-700">Book Cover name</label>
                                            <input type="text" name="bookCover" value="${bookCover.cover_type}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                        </div>
                                    </div>
                                </div>
